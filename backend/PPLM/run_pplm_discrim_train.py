...
class Discriminator(torch.nn.Module):
    ...
    def forward(self, x, length_x):
        ...
        logits = self.classifier_head(avg_hidden, length_x)
...
def collate_fn(data):
    ...
    x_batch, length_batch = pad_sequences(item_info["X"])
...
def train_epoch(data_loader, discriminator, optimizer,
    ...
    for batch_idx, (input_t, target_t, length_t) in enumerate(data_loader):
        input_t, target_t, length_t = input_t.to(device), target_t.to(device), length_t.to(device)

        optimizer.zero_grad()

        output_t = discriminator(input_t, length_t)
    ...
    if dataset == "SST":
        ...
        with open("datasets/clickbait/clickbait_train_prefix.txt") as f:
            for i, line in enumerate(tqdm(f, ascii=True)):
                try:
                    ...
                    if len(seq) < min_length_seq:
                        print("Line {} is shorter than maximum length {}".format(
                            i, min_length_seq
                        ))
        ...
        with open(dataset_fp) as f:
            ...
            for row in tqdm(csv_reader, ascii=True):
                if row:
                    classes.add(row['raw_character_text'])
