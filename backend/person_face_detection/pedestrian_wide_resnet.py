...
if __name__ == '__main__':
    ...
    adm = Adam(lr=0.0001)
    model = WideResNet(128, 128, depth=16, k=6).create_model()
    # model.compile(optimizer=adm, loss={'output_g': "categorical_crossentropy",
    #                                    'output_a': "categorical_crossentropy"},
    #               loss_weights={'output_g': 1.,
    #                             'output_a': 1.},
    #               metrics=['accuracy'])
    model.compile(optimizer=adm, loss={'output_g': "categorical_crossentropy"}, metrics=['accuracy'])
    ...
    callbacks = [
                 ModelCheckpoint('pedestrian_weights.{epoch:02d}-{val_loss:.2f}.hdf5',
                                 monitor='val_acc',
                                 verbose=1,
                                 save_best_only=True,
                                 mode='auto'),
    ...
    model.fit([X_train], [y_g_train], batch_size=batch_size, shuffle=True, validation_split=0.15,
              nb_epoch=nb_epochs,
              callbacks=callbacks)
