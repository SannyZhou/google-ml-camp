...
def get_val_data(val_data_path):
    ...
    agedb_30, agedb_30_issame = get_val_pair(val_data_path, 'agedb_30')
...
def perform_val(multi_gpu, device, embedding_size, batch_size, backbone, carray, issame, nrof_folds = 10, tta = True):
    ...
    with torch.no_grad():
        while idx + batch_size <= len(carray):
            ...
            if tta:
                ...
                emb_batch = backbone(ccropped.to(device)).cpu() + backbone(fliped.to(device)).cpu()#elementwise
    ...
    #tpr,fpr in all test_set pics with different thresholds
    #best thresholds in certain training set(precision highest)and evaluate it on test_set
...
def accuracy(output, target, topk=(1,)):
    ...
    _, pred = output.topk(maxk, 1, True, True)#values,indices (batch_size,maxk)

    pred    = pred.t()#transpose(maxk,batch_size)
    correct = pred.eq(target.view(1, -1).expand_as(pred))#(1,batch_size)expand to (maxk,batch_size)
    #after eq:(maxk,batch_size)
    print("pred:"+str(pred.data))
    print("label:"+str(target.data))
    res = []
    for k in topk:
        correct_k = correct[:k].view(-1).float().sum(0)#(k,batch_size)->(k*batch_size) max_correct_k=bs
        res.append(correct_k.mul_(100.0 / batch_size))#how many batch in percentage can be correct
