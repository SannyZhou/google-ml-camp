...
class Goldfish(object):
    ...
    VIDEO = '/Users/Sanny02/Desktop/test.mp4'
    # VIDEO = 0
    ...
    def __init__(self, depth=16, width=8, face_size=64):
        ...
        # todo pathdir modify
        self.pedstrian_detection_yolo = YOLO()
    ...
    def detect_pedestrian(self):
        ...
        # fps = 10
        output_text = open(self.VIDEO.split('.')[0] + '.txt', 'w')
        ...
        while True:
            ...
            # todo time of this frame in the video (video_capture.get(?))
            ...
            # return boxes of persons
            boxes = self.pedstrian_detection_yolo.detect_image(image)
            ...
            for person_det in detections:
                bbox = person_det.to_tlbr()
                pedestrians.append([0, 1.0].extend(bbox))
                center_x, center_y, aspect_ratio_person, height_person = person_det.to_xyah()

            pedestrians = np.asarray(pedestrians)
            ...
            durr = float(toc - tic)
            fps = 1.0 / durr
            cv2.putText(frame, "fps: %.3f"%fps, (20, 20), 2, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
